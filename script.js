...
function importMaterial(data) {
  ...
  // Convert incoming blocks to nb-style blocks used by the content system
  const rawBlocks = Array.isArray(data.blocks) ? data.blocks : [];

  // Build nb-compatible blocks (content field for text, url for media)
  const blocks = rawBlocks.map((b, i) => {
    ...
    // Text blocks: h1, h2, h3, p, quote, callout, code
    const text = b.text || b.content || '';
    return { id, type: b.type, content: text, url:'', caption:'', name:'' };
  }).filter(Boolean);

  // Convert blocks to legacy format (body, images, videoUrl, files)
  // — this is what theoryAccordionHTML reads
  const textBlocks = blocks.filter(b => ['p','h1','h2','h3','quote','callout','code','divider'].includes(b.type));
  const imgBlocks  = blocks.filter(b => b.type === 'image');
  const vidBlocks  = blocks.filter(b => b.type === 'video');
  const fileBlocks = blocks.filter(b => b.type === 'file');
  ...
  const item = {
    ...
    isLibrary: true,
    date: new Date().toLocaleDateString('ru'),
    // Legacy fields read by theoryAccordionHTML
    body,
    images,
    videoUrl,
    files,
    // Full blocks array for the block editor (round-trip)
    blocks,
