...
// 3. Защита от брутфорса (блокировка отключена)
const _loginAttempts = {};
function checkBruteForce(login){ return true; }
function recordFailedLogin(login){}
function resetLoginAttempts(login){ delete _loginAttempts[login]; }
...
async function initData(){
  ...
  if(!existingUsers.length){
    // Создаём пользователей с хешами паролей
    const h1 = await hashPassword('admin123');
    const h2 = await hashPassword('1234');
    const h3 = await hashPassword('1234');
    save('users',[
      {id:'admin', login:'admin', passwordHash:h1, password:'admin123', name:'Преподаватель', role:'admin'},
      {id:'anna',  login:'anna',  passwordHash:h2, password:'1234', name:'Анна Петрова',  role:'student', subject:'Биология', active:true},
      {id:'dima',  login:'dima',  passwordHash:h3, password:'1234', name:'Дмитрий Козлов',role:'student', subject:'Химия',    active:true}
    ]);
  } else {
    // Пересчитываем хеши у всех у кого есть plain-text пароль (на случай смены алгоритма)
    const needsRehash = existingUsers.filter(u => u.password);
    if(needsRehash.length){
      const hashes = await Promise.all(needsRehash.map(u => hashPassword(u.password)));
      needsRehash.forEach((u, i) => { u.passwordHash = hashes[i]; });
      save('users', existingUsers);
    }
  }
...
function doLogin(){
  ...
  hashPassword(upass).then(hash=>{
    const ok = (found.passwordHash && hash === found.passwordHash)
             || (found.password && found.password === upass);
    if(!ok){
