...
function doLogin(){
  ...
  const users  = load('users')||[];
  // Поддерживаем оба формата: старый (plain) и новый (hash)
  const user = users.find(u=>{
    if(u.login!==uname) return false;
    if(u.passwordHash) return false; // хеш проверяем async ниже
    return u.password===upass; // временно для старых аккаунтов
  });
  // Async-проверка хеша
  const userWithHash = users.find(u=>u.login===uname && u.passwordHash);
  if(!user && !userWithHash){
  ...
  if(userWithHash){
    hashPassword(upass).then(hash=>{
      if(hash!==userWithHash.passwordHash){
        recordFailedLogin(uname);
        document.getElementById('login-err').textContent='Неверный логин или пароль';
        return;
      }
      resetLoginAttempts(uname);
      _startSession(userWithHash);
    });
    return;
  }
  resetLoginAttempts(uname);
...
(async () => {
  ...
  try {
    ...
    overlay.innerHTML = `
      <div style="font-family:'Playfair Display',serif;font-size:2rem;color:var(--green-deep)">Био<span style="color:var(--chem)">Хим</span></div>
      <div style="color:var(--red);font-size:0.9rem;max-width:340px;text-align:center">⚠️ Ошибка инициализации:<br><b>${e.message||e}</b></div>
    `;
    return;
  }
